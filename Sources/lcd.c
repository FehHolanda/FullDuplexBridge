...
//Fun��o que executa o pulos de ENABLE
void Enable(void){
...
void puts_LCD(unsigned char *word){
	...
	while(word[i] != '\0'){
		...
		if(i>=32){			
			setpos_LCD(1,p);					//seleciona a posi��o fantasma(17) do diplay
			putchar_LCD(word[p-1]);				//Salva na posi��o fantasma	o primeiro caracter da linha 2
			putcmd_LCD(0b0000011000,40);		//Shift no display
			setpos_LCD(2,p);					//seleciona a ultima posi��o da segunda linha	
